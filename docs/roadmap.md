...
| Installed GUI QA handoff log | Implemented | `docs/qa/mvp-installed-gui-2026-07-05.md` records the current candidate artifact and the remaining human installed-app interaction checklist. |
...
| Current manual QA result stub | Implemented | `docs/qa/mvp-manual-qa-7e14f0a-2026-07-05.md` is prefilled with current candidate metadata and the remaining installed-app GUI checks. |
...
| CI artifact log generator | Implemented | `npm run qa:artifact-log -- <workflow-run-id> <commit-sha>` writes the CI artifact confirmation from GitHub run metadata. |
| Human QA packet orchestrator | Implemented | `npm run qa:ready-for-human -- <workflow-run-id> <commit-sha>` prepares and validates the full candidate packet in one command. |
